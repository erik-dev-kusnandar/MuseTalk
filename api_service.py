...
timesteps    = None
weight_dtype = None
avatar_cache = {}  # Cache loaded avatars: {avatar_id: AvatarObject}

# ─── Avatar class (disederhanakan dari realtime_inference.py) ──
class Avatar:
    def __init__(self, avatar_id: str, batch_size: int = 20):
        self.avatar_id   = avatar_id
        # FIXED: Use avatar_id for base_path, not the global AVATAR_PATH
...
async def startup_event():
    ...
    try:
        avatar_cache[AVATAR_ID] = Avatar(avatar_id=AVATAR_ID, batch_size=20)
...
async def prepare_avatar(file: UploadFile = File(...)):
    ...
    try:
        ...
        config_data = {
            avatar_id: {
                "preparation": True,
                "bbox_shift": 0,  # Set to 0 for v1.5 to avoid over-cropping
        ...
        # Reload/Add to cache
        logger.info(f"Loading newly prepared avatar: {avatar_id}")
        avatar_cache[avatar_id] = Avatar(avatar_id=avatar_id, batch_size=20)
...
async def health():
    return {
        "status": "ready" if avatar_cache else "loading",
        "device": str(device),
        "avatar": AVATAR_ID,
    }
...
async def generate_video(audio_file: UploadFile = File(...), avatar_id: Optional[str] = None):
    ...
    if target_id not in avatar_cache:
        logger.info(f"Avatar {target_id} not in cache, trying to load...")
        try:
            avatar_cache[target_id] = Avatar(avatar_id=target_id, batch_size=20)
        except Exception as e:
            logger.error(f"Failed to load avatar {target_id}: {e}")
            raise HTTPException(404, f"Avatar {target_id} not found or invalid")
    ...
    logger.info(f"Processing audio chunk: {audio_file.filename} (Target Avatar: {target_id})")
    ...
    try:
        ...
        # Serialize inference and prevent event loop blocking
        async with inference_lock:
            output_path = await asyncio.to_thread(obj.inference, tmp_audio, out_name, fps=25)
            
        logger.info(f"Video generated for {target_id} in {time.time()-t0:.1f}s → {output_path}")
